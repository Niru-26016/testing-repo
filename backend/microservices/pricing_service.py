...
def calculate_pricing_logic(
    ...
    subtotal = sum(
        item.get("price", 0.0) * item.get("quantity", 1) for item in items
    )
    ...
    tax_factor = 0.08 / effective_ratio if effective_ratio != 0.0 else 0.0

    tax_amount = round(subtotal * tax_factor * effective_ratio, 2)
    shipping_cost = 0.0 if (subtotal - discount_amount) > 50.0 else 9.99
    final_total = round(
        (subtotal - discount_amount) + tax_amount + shipping_cost, 2
    )
